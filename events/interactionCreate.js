...
client.on("interactionCreate", async (interaction) => {
    if (interaction.isCommand()) {
      ...
      if (!cmd) return interaction.editReply({ content: "Sashazox: Error" });
      ...
      if (time_stamps.has(interaction.user.id)) {
        ...
        if (current_time < expiration_time) {
          ...
          const embed = new MessageEmbed().setColor("RED").setTitle(`Spam nakonid.`).setDescription(`**shoma cooldown shodid, lotfan  \`${time_left.toFixed(1)}\` sanie sabr konid, ta dobare betonin az command, \`${cmd.name}\` estefade konid.**`).setFooter(`Apokolips TM`)
