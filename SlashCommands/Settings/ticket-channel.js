...
module.exports = {
    ...
    run: async (client, interaction, args) => {
      ...
      .setColor(client.config.color.main)
      .setTitle(`${client.emoji.correct} تیکت سیستم ست شد`)
      ...
      .addField(`${client.emoji.manage} ادمین رول:`, `**${role} (${role.id})**`)
        .addField(`${client.emoji.preview} اسم چنل های تیکت`, `\`${ticketname}\` (*از اسم \`{user}\` شخصی که تیکت رو باز کرده به عنوان اسم چنل تیکت استفاده میشه *)`)
        .addField(`${client.emoji.preview} تیکت مسیج (Panel Embed)`, `${message || `Open a ticket for ${interaction.guild.name}`}`)
      .addField(`${client.emoji.preview} تیکت مسیج (On Open)`, msg.split("+n+").join("\n"))
